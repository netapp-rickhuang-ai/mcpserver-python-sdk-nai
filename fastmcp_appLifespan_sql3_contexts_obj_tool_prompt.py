...
mcp = FastMCP("File Ingestion App") # [NOTE][TODO] add ANY dependencies=List["libraries"] ???
...
def calculate_bmi(weight_kg: float, height_m: float) -> float:
    ...
    logger.info(f"[INFO] Calculated BMI: {bmi:.2f} for weight: {weight_kg} kg and height: {height_m} m")
    return bmi
...
async def fetch_weather(city: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        logger.info(f"[INFO] Fetched weather for city: {city}, Response: {response.text}")
...
async def run_app_lifespan():
    ...
    async with app_lifespan(mcp) as context:
        ...
        print(f"Read files: {context.files}")  # Example: Print database name or other context attributes
...
test_database="/home/ailab/files/mcp-server-demo/sqlite3/test_database.sqlite"
